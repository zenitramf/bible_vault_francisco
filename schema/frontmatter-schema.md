...
## Catalog-derived fields (not page frontmatter)

`wiki/catalog.jsonl` is generated by `wiki_tool.py build`. Rows include page frontmatter **plus** derived fields for agent retrieval. Derived fields are **not** written back into wiki pages.

| Catalog field | Origin |
|---|---|
| `path`, `title`, `type`, `tags`, `status`, `updated`, `description`, `source_count` | Page frontmatter |
| `bible_reference`, `bible_book_key`, `bible_book_name` | Page frontmatter (when present) |
| `bible_references` | Primary frontmatter ref + display refs in `## Biblical passages` |
| `bible_book_keys` | Derived from `bible_references` (+ frontmatter book key) |
| `source_paths` | Wikilinks to `sources/**` and optional `source_path` |
| `related_paths` | Wikilinks to `wiki/**` |
| `primary_source_path` | Frontmatter `source_path` on source-notes |
| `headings` | Markdown headings in the page body |
| `aliases` | Optional frontmatter `aliases` list only (never invented by build) |

Reverse indexes under `wiki/indexes/` (`by-tag`, `by-passage`, `by-source`, `by-type`) are also generated on `build`. Do not hand-edit them.
