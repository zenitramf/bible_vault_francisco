...
**Side-by-side Obsidian Bases:** every directory with `index.md` also has a paired `index.base` (live folder database). Exception: the **vault root** has only `bases.base` (the base of bases enumerating every `.base` file)—never a root `index.base`. Bases are counterparts to static OKF lists; they do not replace `index.md`.

Regenerate **wiki** folder indexes, the catalog, reverse indexes, and all `index.base` / root `bases.base` files with:

```bash
python3 .tools/scripts/wiki_tool.py build
```

Source-tree `index.md` files remain hand-maintained when sources move (never rewrite source document bodies). Paired `index.base` files under `sources/` (and elsewhere) are machine-regenerated by `build`.
